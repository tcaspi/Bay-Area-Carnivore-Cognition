...
```{r}
data <- read.csv("Data/Q1_Summary_Firsts_DummyRows_Max_07282025_Anonymized.csv") %>%  # first visits only
...
p.vigilance <- ggplot(posterior_plotting, aes(y = Species, x = mean_prob, color = Species, fill=Species)) +
  # 95% interval
   geom_errorbarh(aes(xmin = lower_95, xmax = upper_95),
                 height = 0.1, size = 0.5) +
  # 90% interval
  geom_linerange(aes(xmin = lower_90, xmax = upper_90), alpha = 1, size = 5) +
  # Mean point
  geom_point(shape = 16, size = 1, color = "white") +
